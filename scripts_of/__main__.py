...
from . import blast_file_processor, files, mcl, util, matrices, orthologues, program_caller, trees_msa, split_ortholog_files, synteny
...
def WriteGraph_perSpecies(args):
    ...
    with open(graphFN + "_%d" % iSpec, 'w') as graphFile:
        ...
        B_connect: list[sparse.csr_matrix] = matrices.MatricesAnd_s(connect2, B)
        del B, connect2
...
class WaterfallMethod:    
    ...
    @staticmethod
    def ConnectCognates(options, seqsInfo, iSpecies, d_pickle): 
        ...
        if(options.synteny):
            synteny_matrices = matrices.LoadMatrixArray('SYN', seqsInfo, iSpecies, d_pickle)
            connect = [con_mat.maximum(syn_mat) for con_mat, syn_mat in zip(connect, synteny_matrices)]
...
#    print("                 Options: of_recon, dlcpar, dlcpar_convergedsearch")
    ...
    print(" -syn <dir>      The synteny file for connect cognates, format .anchors")
...
def ProcessArgs(prog_caller, args):
    ...
    while len(args) > 0:
        ...
        if arg == "-f" or arg == "--fasta":
            ...
            continuationDir = GetDirectoryArgument(arg, args)
        elif arg == "-syn" or arg == "--synteny":
            if options.qStartFromTrees:
                print("Repeated argument: -syn/--synteny\n")
                util.Fail()
            options.synteny = True
            syntenyDir = GetDirectoryArgument(arg, args) 
    ...
    return options, fastaDir, continuationDir, syntenyDir, resultsDir_nonDefault, pickleDir_nonDefault            
...
def DoOrthogroups(options, speciesInfoObj, seqsInfo):
    ...
    cmd_queue = mp.Queue()
    for iSpecies in range(seqsInfo.nSpecies):
        cmd_queue.put((options, seqsInfo, iSpecies))
...
def main(args=None):    
    try:
        ...
        options, fastaDir, continuationDir, syntenyDir, resultsDir_nonDefault, pickleDir_nonDefault = ProcessArgs(prog_caller, args)  
